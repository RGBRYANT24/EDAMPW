...
int CLayout::CanBePutIn(CPart Part)
{
	...
	/*else
	{
		if (nPutInAmount == 1)   //ֻ
		{
			if (dSurplusAreaWithoutTrans == 0) return 1;
			else if (dSurplusAreaWithTrans == 0) return -1;
			else return pow(-1, RND(0, 1));
		}
...
void CLayout::Merge()
{
	...
	for (; itUseless != m_lstUseless.end(); itUseless++)
	{
		...
		//判断左右相邻的情况
		//
		/*else if (dAvailY >= dUselessY)//改成大于是存在左侧废弃空间细长，将可用部件左推。但是需要增加条件判断相邻
		{
			//废弃在左，可用在右
			...
			if (dUselessX + dUselessWidth == dAvailX)//判断相邻
			{
				...
				if (dUselessHeight+dUselessY >= dAvailHeight+dAvailY)//合并条件，更新合并条件
				{
					itAvail->setX(dUselessX);
					itAvail->setWidth(dUselessWidth + dAvailWidth);//实际上就是相当于加宽空间
					//if (dUselessHeight - dAvailHeight > 0)//原先只考虑了分离之后上面有废弃空间的情况
					if (dUselessHeight + dUselessY - dAvailHeight - dAvailY > 0)//现在要考虑上下都有废弃空间.这是上面有废弃空间
					{
						CAreaList tmpArea;
						tmpArea.setHeight(dUselessHeight + dUselessY - dAvailHeight - dAvailY);
						tmpArea.setWidth(dUselessWidth);
						tmpArea.setX(dUselessX);
						tmpArea.setY(dAvailY+dAvailHeight);
						m_lstUseless.erase(itUseless);
						m_lstUseless.push_back(tmpArea);
					}
					if (dAvailY - dUselessY > 0)//下面分割出废弃空间
					{
						CAreaList tmpArea;
						tmpArea.setHeight(dAvailY - dUselessY);
						tmpArea.setWidth(dUselessWidth);
						tmpArea.setX(dUselessX);
						tmpArea.setY(dUselessY);
					}
		...
		else if (dAvailY == dUselessY)
		{
			...
			/*if (dUselessX + dUselessWidth == dAvailX)//判断相邻
			{
				...
				if (dUselessHeight >= dAvailHeight)//合并条件
				{
					...
					itAvail->setWidth(dUselessWidth + dAvailWidth);//实际上就是相当于加宽空间
					//if (dUselessHeight - dAvailHeight > 0)//原先只考虑了分离之后上面有废弃空间的情况
					if (dUselessHeight - dAvailHeight  > 0)//现在要考虑上下都有废弃空间.这是上面有废弃空间
					{
						CAreaList tmpArea;
